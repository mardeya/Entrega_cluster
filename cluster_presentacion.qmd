...
Cabe destacar que tanto China como la India son outliers por poblacion. (Superpoblación). Son los números 79 y 33.
...
## K-medoid clustering
Utilizaremos 3 clusters.
```{r}
library(cluster)
set.seed(123)
pam_clusters <- pam(qual, k=3, metric = "manhattan")
fviz_cluster(pam_clusters, datos, ellipse.type = "t", repel = TRUE) +
  theme_bw() +
  theme(legend.position = "none")
```
...
```{r, results= FALSE}
mat_dist <- dist(qual, method = "manhattan")  # Distancia euclidíana
